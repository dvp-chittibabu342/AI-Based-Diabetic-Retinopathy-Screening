...
class TestLesionSegmentation(unittest.TestCase):
    ...
    def test_04_pytorch_checkpoint_presence(self):
        """Verify best_model.pt checkpoint integrity if present."""
        if not self.model_pt.exists():
            self.skipTest(f"PyTorch checkpoint not bundled at {self.model_pt}; production runtime uses ONNX (lesion_unet.onnx)")
